...
Upon trying to decipher this text, I realised substitution and shift ciphers do not work. That turned me to Vigenere cipher since the flag, `picoCTF`, seemed to correspond to `pohzCZK`. As seen, `p` and `C` are remained the same, so I deduce that they key could be of length 4.

Using [this online tool](https://www.guballa.de/vigenere-solver), the key is indeed of length 4, `flag`, and I obtained the plaintext.
